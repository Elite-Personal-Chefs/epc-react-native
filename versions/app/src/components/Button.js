...
function GoToButton({
	...
	return (
		<TouchableOpacity
			style={[globalStyles.navigate_away, noBorderBottom && { borderBottomWidth: 0 }]}
			onPress={() => navigation.navigate(navigator, { screen: page, details: params })}
		>
...
function SubmitButton({ text, onPress, style, disabled }) {
	return (
		<TouchableOpacity
			activeOpacity={0.8}
			onPress={onPress}
			style={[styles.submitButtonContainer, disabled && styles.submitButtonDisabled]}
		>
			<Text
				style={[
